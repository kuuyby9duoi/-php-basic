...
<a class="btn btn-primary btn-sm d-none d-sm-inline-block" name="export" role="button" title="xuất ra file Excell" href="acc.php?acc=export&id=0"><i class="fas fa-download fa-sm text-white-50"></i>&nbsp;Xuất .xlsx</a>
				...
				<?php $run=mysqli_query($conn,"select * from khachhang"); $num=mysqli_num_rows($run);  if($page==1) {$start=1; } else {$start=$page*$sl-$sl+1;}  ; $run2=mysqli_query($conn,"select * from khachhang limit $start,$sl");while ($row_option=mysqli_fetch_array($run2)) {;?>
				<tr id="body_table">
